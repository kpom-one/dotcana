# AI Training Approach for Lorcana

## Executive Summary

Memory-based game AI using similarity lookup instead of neural networks. Two complementary views: board state similarity (tactical) and card trajectory similarity (strategic). No gradient descent, no model training - just record games, vectorize states, and look up what worked.
...
### 1. Board State Vectors (Tactical)

Snapshot of the current game position.

```
Features:
- Player lore (both players)
- Ink available/total
- Cards in hand/play/inkwell (counts)
- Board strength/willpower totals
- Exerted character counts
- Turn number
```

**Query**: "In similar board positions, what action won most often?"

**Horizon**: This turn. Immediate tactical response.

### 2. Trajectory Vectors (Strategic)

A card's journey through the game: where it's been, where it's going.

```
Trajectory = sequence of states:
  [hand, hand, play, play, play, exerted, damaged, discard]

Vectorize by encoding each timestep:
  - Zone (hand/play/inkwell/discard)
  - Exerted (0/1)
  - Damage amount
  - Turns in current zone
```

**Query**: "Cards with similar life arcs - how did their stories end?"
...
- State space is small enough to cover (Lorcana: ~50-100 features)
...
Cards have stable IDs (`p1.elsa_spirit_of_winter.a`) that persist across states. Enables trajectory tracking without inference.
